...
import Cardano.Api.Shelley (Address (ShelleyAddress), AsType (AsAlonzoEra), Lovelace (Lovelace), ProtocolParameters, fromPlutusData, fromShelleyStakeReference, shelleyPayAddrToPlutusPubKHash, toShelleyStakeAddr, toShelleyStakeCredential)
-- import qualified Plutus.Contracts.V1.SimpleMarketplace as DirectSale

import qualified Cardano.Api.Shelley as Shelley
import Cardano.Kuber.Api
import Cardano.Kuber.Data.Parsers (parseAssetIdText, parseAssetNQuantity, parseScriptData, parseValueText)
...
mint ctx signKey addrEra policyId assetName amount= do
  ...
  case txBodyE of
    ...
    Right txBody -> do
      ...
      -- putStrLn $ "Wait mint to appear on wallet TxId: " ++ show txId
    --   pollForTxId ctx (getAddrAnyFromEra addrEra) txId
